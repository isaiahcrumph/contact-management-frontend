// src/pages/EmptyContactsPage.jsx
...
const EmptyContactsPage = () => {
  ...
  const handleUpdate = async () => {
    ...
    try {
      setLoading(true);
      const contact = await emptyContactService.getEmptyContactById(selectedContactId);
  ...
  const confirmDelete = async () => {
    try {
      setLoading(true);
      await emptyContactService.deleteEmptyContact(selectedContactId);
  ...
  const handleFormSubmit = async (formData) => {
    try {
      ...
      if (formData.id) {
        // Update
        await emptyContactService.updateEmptyContact(formData);
        setMessage({ type: 'success', message: 'Contact updated successfully' });
      } else {
        // Create
        await emptyContactService.createEmptyContact(formData);
  ...
  const handleViewContact = async (id) => {
    ...
    try {
      ...
      const contact = await emptyContactService.getEmptyContactById(id);
  ...
  return (
    <PageLayout>
      ...
      <h1 className="text-3xl font-bold text-white text-center mb-8">Empty Contacts Management</h1>
